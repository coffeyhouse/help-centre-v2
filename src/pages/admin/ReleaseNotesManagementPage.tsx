...
export default function ReleaseNotesManagementPage() {
  ...
  const loadData = async () => {
    try {
      ...
      const response = await fetch(`/api/files/${region}-release-notes`, {
        headers: {
      ...
      if (!response.ok) {
        throw new Error('Failed to load release notes data');
      }
  ...
  const handleSave = async () => {
    try {
      ...
      if (!response.ok) {
        throw new Error('Failed to save release notes data');
      }
    ...
    >
      <div className="mb-6">
        ...
        <p className="text-gray-600">
          Manage product release notes for {regionName}
        </p>
      </div>
